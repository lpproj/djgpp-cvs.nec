...
void (*__libc_itimer_pf_handle_itimer)(void);
void (*__libc_itimer_pf_unhandle_itimer)(void);

static long default_clock_tick_interval_microsec;

static int __itimer_bss_count = -1;
static void itimer_setup_subfuncs(void);
static inline void itimer_setup(void)
{
  if (__itimer_bss_count != __bss_count) {
    __itimer_bss_count = __bss_count;
    itimer_setup_subfuncs();
  }
}

#define DEFAULT_CLOCK_TICK_INTERVAL default_clock_tick_interval_microsec
...
static
void
itimer_setup_subfuncs(void)
{
  switch(__crt0_machine_type)
  {
#if defined SUPPORT_IBMPC
    case MACHINE_TYPE_IBMPC:
      __libc_itimer_pf_handle_itimer = __libc_itimer_handle_itimer_ibmpc;
      __libc_itimer_pf_unhandle_itimer = __libc_itimer_unhandle_itimer_ibmpc;
      default_clock_tick_interval_microsec = 54925 + 1;
      break;
#endif
#if defined SUPPORT_NEC98
    case MACHINE_TYPE_NEC98:
      __libc_itimer_pf_handle_itimer = __libc_itimer_handle_itimer_nec98;
      __libc_itimer_pf_unhandle_itimer = __libc_itimer_unhandle_itimer_nec98;
      default_clock_tick_interval_microsec = 10000 + 1;
      break;
#endif
    default:
      ABORT_UNSUPPORTED;
